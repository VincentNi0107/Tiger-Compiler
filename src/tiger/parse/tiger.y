...
decs:  {$$ = new absyn::DecList();}
  |  dec_one  {$$ = new absyn::DecList($1);}
  |  dec_one decs  {$$ = ($2)->Prepend($1);}
  ;

dec_one:  tydec {$$ = new absyn::TypeDec(scanner_.GetTokPos(), $1);}
  |  vardec  {$$ = $1;}
...
sequencing_exps:  {$$ = new absyn::ExpList();}
  |  exp  {$$ = new absyn::ExpList($1);}
  |  exp SEMICOLON sequencing_exps  {$$ = ($3)->Prepend($1);}
  ;

sequencing:  exp SEMICOLON exp  {absyn::ExpList *tmp = new absyn::ExpList($3); $$ = tmp->Prepend($1);}
